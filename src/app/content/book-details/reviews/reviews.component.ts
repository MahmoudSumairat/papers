...
export class ReviewsComponent implements OnInit {
  ...
  onLike(
    ...
    if(this.likeFlag) { //Check if the button is available to click or not to aviod the setTimeOut function issues
       //Check if the selected button span innerText is Like which means that the user didn't click the like
    if (!this.spanTextsIds.includes((<HTMLSpanElement>document.getElementById('btn-' + username)).id)) {
      ...
      if (this.isAuth) {
        ...
        if (userLikes) { //Check if the likes Array in the user document is defined or NOT
          const newArr = [
            ...userLikes,
        ...
        } else { //If the likes array is NOT defined
          this.afs
            .collection("stars") //Collection Stars in the database
            .doc("book_review") //Document Book Review in the collection Stars
            .collection(this.bookName.toLowerCase().replace(/ /g, "_")) //Select the collection that depends on the bookName that i got from the URL
            ...
            }, {merge : true}); //Allow to merge the current fields with this field
        }
      } else { //the user is not signed in so he should go to the sign in page and sign in
    ...
    setTimeout(() => { //I implemented this function because the valueChanges Observable send a new markup when the data changes so i should wait tell it finish and modify it so i can modify the newest markup
      ...
      //Again check if the user clicked like or not
        if(!this.spanTextsIds.includes((<HTMLSpanElement>document.getElementById('btn-' + username)).id)) {
          ...
          this.spanTextsIds.splice(this.spanTextsIds.indexOf((<HTMLSpanElement>document.getElementById('btn-' + username)).id),1);
      ...
      this.spanTextsIds.forEach(spanId => {
        newSpanTexts.push(
          (<HTMLSpanElement>document.getElementById(spanId))
        )
      })
      ...
      (<HTMLSpanElement>document.getElementById('btn-' + username))

      this.likeFlag = false;
    }, 20);
    }

    setTimeout(() => {this.likeFlag = true}, 300);
   
  }

  toggleCommentBox(commentBox: ElementRef) {
    //Show and hide the comment box when click on the comments button
    if (this.boxShown) {
      //check if the box is shown or not
      this.boxShown = !this.boxShown; //Toggle the flag for the next time
      this.renderer.setStyle(commentBox, "display", "none"); //Set the style to hide the box if it is shown
    } else {
      this.boxShown = !this.boxShown; //Toggle the flag for the next time
      this.renderer.setStyle(commentBox, "display", "block"); //Set the style to show the box if it is hidden
    }
  }

  onSubmitComment(form: NgForm, username: string, comments) {
    //Send the comment into the database
    if (this.isAuth) {
      //Check if the user is signed in
      this.afs
        .collection("stars") //Select collection Stars
        .doc("reviewes_comments") //Selects document Reviewes Comment
        ...
        .set(
          {
            content: form.value.comment, //Set the commetn content into the content field
            name: this.user.userName.replace(/@([^.@\s]+\.)+([^.@\s]+)/, "") //Set the username into the name field
          },
  ...
  getLikeOrUnlikeTitle(userLikes : string[], btn) {
    if(this.isAuth) { //Check if the user has signed in to show him his likes
      if(userLikes) { //Check if the userLikes array is defined or not
        if(userLikes.includes(this.user.userName.replace(/@([^.@\s]+\.)+([^.@\s]+)/, ""))) { //If the username is included in the likes array that means he clicked the like button\
          if(!this.spanTextsIds.includes(btn.id)) { //Check if the spansTextIds array does not include the liked buttons
    ...
    } else { //There are no user logged in so i show the like button to the guest
      return 'Like';
    }
  }
}
